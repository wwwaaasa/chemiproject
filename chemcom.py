...
# ===== 데이터 불러오기 =====
csv_path = r"C:\Users\samsung\OneDrive\문서\수행평가\화학 수행\3학년\dataset\energy2(H).txt"
...
ry['E_dirac'] = ry.apply(lambda r: E_dirac(r['n'], r['J']), axis=1)
...
# Schrödinger 에너지 (Z 적용)
def E_schr_general(Z, n):
    return -Rhc * Z**2 / n**2
...
# H I 준비
df_plot_abs_H = df_plot_abs.copy()
df_plot_abs_H['DeltaE'] = np.abs(df_plot_abs_H['E_schr']) - np.abs(df_plot_abs_H['E_dirac'])

# Na I 준비
df_plot_abs_Na = ry_valid.copy()  # Na I에서 ry_valid 사용 중
df_plot_abs_Na['DeltaE'] = np.abs(df_plot_abs_Na['E_schr']) - np.abs(df_plot_abs_Na['E_dirac'])

# 그래프 그리기
plt.figure(figsize=(10,6))

# H I
plt.plot(df_plot_abs_H['n'], np.abs(df_plot_abs_H['DeltaE']), 
         marker='o', color='magenta', linestyle='-', linewidth=2, markersize=8, label='ΔE (H I)')

# Na I
plt.plot(df_plot_abs_Na['n'], np.abs(df_plot_abs_Na['DeltaE']), 
         marker='s', color='cyan', linestyle='-', linewidth=2, markersize=8, label='ΔE (Na I)')

# y축 log scale
plt.yscale('log')

# 라벨/제목
plt.xlabel('주양자수 n', fontsize=12)
plt.ylabel('ΔE (eV)', fontsize=12)
plt.title('ΔE vs n (Schrödinger - Dirac) 비교: H I vs Na I (log scale)', fontsize=14)
...
# Na I 준비
df_plot_abs_Na = ry_valid.copy()

# 그래프 그리기 → E vs n (log scale)

plt.figure(figsize=(10,6))

# H I - Schrödinger
plt.plot(df_plot_abs_H['n'], np.abs(df_plot_abs_H['E_schr']), 
         marker='o', color='magenta', linestyle='-', linewidth=2, markersize=8, label='E_schr (H I)')

# H I - Dirac
plt.plot(df_plot_abs_H['n'], np.abs(df_plot_abs_H['E_dirac']), 
         marker='^', color='purple', linestyle='-', linewidth=2, markersize=8, label='E_dirac (H I)')

# Na I - Schrödinger
plt.plot(df_plot_abs_Na['n'], np.abs(df_plot_abs_Na['E_schr']), 
         marker='s', color='cyan', linestyle='-', linewidth=2, markersize=8, label='E_schr (Na I)')

# Na I - Dirac
plt.plot(df_plot_abs_Na['n'], np.abs(df_plot_abs_Na['E_dirac']), 
         marker='v', color='blue', linestyle='-', linewidth=2, markersize=8, label='E_dirac (Na I)')
...
# 라벨/제목
plt.xlabel('주양자수 n', fontsize=12)
plt.ylabel('에너지 E (eV)', fontsize=12)
plt.title('E vs n (Schrödinger / Dirac) 비교: H I vs Na I (log scale)', fontsize=14)
